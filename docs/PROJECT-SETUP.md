...
- Confirm the extension appears in the list and its icon shows in the toolbar (pin it if needed).

## Enable and Use Chrome Built‑in AI (Gemini Nano)

- **Requirements (summary)**

- Chrome on desktop (Windows 10/11, macOS 13+, Linux, or Chromebook Plus with recent ChromeOS).
- Storage: ~22 GB free on the Chrome profile volume (model size varies with updates).
- Hardware: GPU with >4 GB VRAM, or CPU with ≥16 GB RAM and ≥4 cores.
- Network: unmetered connection recommended.
- Reference: https://developer.chrome.com/docs/ai/get-started#requirements

- **Enable APIs on localhost**
